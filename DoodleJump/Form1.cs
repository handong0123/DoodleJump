...
int Num,Bn,Gn,Mn,Tn,BTn,Ln;//计数器  Num:砖块 Mn:怪物 Tn:正常模式 BTn:BT模式
struct Doodle
{
	POINT pt;
	int a0,v0,a1,v1;
...
Hbitmap Bkg[4],Green,Blue,Gray,White,Brown[4],Time[8],Duck[5][2],Attack,Bullet,Number[10],Prop_take[5],Prop_help[2][2],Monster[7];
struct MONSTER
{
	POINT pt;
	int kind;//怪兽种类
...
bool UI::MouseInUI(POINT p)//鼠标位置检测
{
...
void Loadmap(HINSTANCE hinstance)//加载位图
{
	int i,j,k;
	char s[10];
	HPEN hp;
...
void Move()
{
	...
	while(p.y>=40)
	{
		...
		while(p.x<=370)
		{
			...
			{
				i=rand()%(100*(3-Tn));//难度调整 1.67% 2.5% 5% 
				if(i==0)
				{
					p.x=(5+rand()%31)*10;
					mstr[Mn].pt=p;
				...
				if(!Gn||j<6)
				{
					...
					else
					{
						...
						else
						{
							...
							else if(i>59-k/3)//60.6>i>57.6   3.3%
							{
								block[Num].kind=4;
								block[Num].n=0;
								TFlag=8;
							}
							else if(i>50-k/3)//57.6>i>48.6 15%
								block[Num].kind=3;
							else if(i>43-k/3)//48.6>i>41.6 11.7%
							{
								block[Num].n=0;
								block[Num].broke=0;
								block[Num].kind=5;
